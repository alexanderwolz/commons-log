...
class LoggerTestLevelWarn : AbstractLoggerTest() {
    ...
    fun testLogLinesWithLevelTrace() {
        ...
        assertEquals("WARN ${javaClass.name} - This is a warning log", logLines[0])
        assertEquals("ERROR ${javaClass.name} - This is an error log", logLines[1])
        assertEquals("ERROR ${javaClass.name} - This is a second error log", logLines[2])
        assertEquals(
